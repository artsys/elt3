...
void ManageBuy() 
{
   ...
   if (li_44 == 0 && StrategySignal() == 1 && IsTradeTime() && !(UseNewsFilter && NewsTime())) 
	{
	   Print("StrategySignal="+(string)StrategySignal());
      if (OpenBuy() == TRUE)
         if (MassHedge) 
				gi_384 = TRUE;
   } 
...
{
   /*
   double r1 = iRSI(NULL,0,6,PRICE_CLOSE,1);
     double r2 = iRSI(NULL,0,6,PRICE_CLOSE,2);
...
bool NewsTime() 
{
   ...
   if (Minute() != PrevMinute && !IsTesting()) 
	{
      PrevMinute = Minute();
      int minutesSincePrevEvent = iCustom(NULL, 1440, "FFCal", true, true, false, true, true, 1, 0);
      int minutesUntilNextEvent = iCustom(NULL, 1440, "FFCal", true, true, false, true, true, 1, 1);
