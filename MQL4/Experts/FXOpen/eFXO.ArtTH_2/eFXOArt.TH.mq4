...
void DelUnused(){
   ...
   //AId_Print2(aTO,aI,4,"aTO_BUY_Above_LastLevel");     
}
...
double getNearestBuyPrice(){
   ...
   string f=StringConcatenate(""
            ...
            ,OE_TY,"==",OP_BUY);
   B_Select(aTO,aB,f);
   //AId_Print2(aTO,aB,4,"aTO_Select_Buy_Above_Ask");  
   
   int aBS[];
   ArrayResize(aBS,0);
   AId_Init2(aTO,aBS,OE_FOOP);
   ...
   f=StringConcatenate(""
            ...
            ,OE_TY,"==",OP_BUYSTOP);
   B_Select(aTO,aBS,f);
   //AId_Print2(aTO,aBS,4,"aTO_Select_BuyStop_Above_Ask");  
   
   AI_Union(aB,aBS);
   AId_InsertSort2(aTO,aB,OE_FOOP);
  // AId_Print2(aTO,aB,4,"aTO_Union_Buy_BuyStop_Above_Ask");
   
   double oop=AId_Get2(aTO,aB,0,OE_FOOP);
   return(oop);          
}
...
double getNearestSellPrice(){
  ...
  // AId_Print2(aTO,aB,4,"aTO_Union_Buy_BuyStop_Above_Ask");
   ...
   double oop=AId_Get2(aTO,aB,(ArrayRange(aB,0)-1),OE_FOOP);
   return(oop);          
}
...
void CMFB(){
   ...
   zx
	...
	while(profit>0&&i<rows){
	   if(aI[i]>=ArrayRange(aTO,0)){
	      i++;
	      continue;
	      DAIdPRINTALL(aTO,"OUT_OF_RANGE_aI_"+aI[i]);
	   }   
		int ti=aTO[aI[i]][OE_TI];
		double opr=aTO[aI[i]][OE_OPR];
...
void TN(){
	...
	*/
	zx
	string fn="TN";
	DAIdPRINTALL3(aTO,__FUNCTION__+"________________");
	//Print(fn);	
	int aI[]; ArrayResize(aI,0,1000);
	AId_Init2(aTO,aI,OE_IM);
	//AId_Print2(aTO,aI,4,"TN_IM1_before");
	
	string f=StringConcatenate(OE_IM,"==1");
 	B_Select(aTO,aI,f);
 	//AId_Print2(aTO,aI,4,"TN_IM1_after");
	...
	for(int i=0;i<rows;i++){
		...
		double 	poop=AId_Get2(aTO,aI,i,OE_FOOP);
	   DPRINT2("pti="+pti);
	   DPRINT2("pty="+pty);
	   DPRINT2("poop="+poop);
...
void TN_checkCO(int pti, int RevKoef=1){
	/**
		...
		\internal
			...
			>Rev:0
	*/
	DAIdPRINTALL3(aOE,__FUNCTION__+"____ti_"+pti);
	zx
	...
	for(int lvl=1;lvl<=Levels;lvl++){
		...
		B_Select(aTO,aI,f);
		DAIdPRINT3(aTO,aI,"lvloop="+lvloop);
		int rows=ArrayRange(aI,0);
		double d[];
		if(rows<=0){
			ArrayResize(d,0);
...
void TN_checkRev(int pti){
	...
	*/
	...
	int pty=OrderType();
	int idx=OE_FIBT(pti);
	double poop=aOE[idx][OE_FOOP];//Norm_symb(OrderOpenPrice());
	double pol=OrderLots();
...
int TI_count(int ty, double oop){
	...
	*/
	...
	AId_Init2(aTO,aI);
	string f=StringConcatenate(""
		,OE_FOOP,">>",Norm_symb(oop)-1*Point
		," AND "
		,OE_FOOP,"<<",Norm_symb(oop)+1*Point
		," AND "
		,OE_TY,"==",ty);
	
	DAIdPRINT2(aTO,aI,"before");	
	B_Select(aTO,aI,f);
...
void Autoopen(){
	...
	*/
	...
	if(OrdersTotal()==0){
		...
		dZeroPrice=OE_getPBT(ti,OE_FOOP);//-(Step/2)*Point;
	}	
}
