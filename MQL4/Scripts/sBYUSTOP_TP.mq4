...
double startPriceToSend = MarketInfo(Symbol(), MODE_BID);
if(PriceStart > 0){
...
for(int i = 0; i < numberOfOrders; i++){
   if(i > 0){
	...
	for(int k=0; k<KolSetok; k++){
		OpenBUYSTOP(startPriceToSend, plusPips, CalcLots(i), sendTP);   
	}	
} 
}
...
int OpenBUYSTOP(double startPrice, int plusPips, double sendLot = 0.1, int tp_pip = 0){
   string osy = Symbol();
   int oty = OP_BUYSTOP;
