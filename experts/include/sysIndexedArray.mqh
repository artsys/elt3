...
double AId_Compare(double v1, double v2){
	/**
		...
		\date		2014.02.25
		\author		Morochin <artamir> Artiom
		\details	¬озвращает результат сравнени€ двух вещественных параметров.
		\internal
			>Hist:	
...
int AId_QuickSearch2(double &a[][], int &aI[], int col=0, double element=0.0, int mode=AI_EQ){
	/**
		...
		\internal
			>Hist:			
					 @0.0.0.3@2014.02.26@artamir	[+]	ƒобавлен поиск любого меншего индекса.
...
int AId_SearchFirst2(double &a[][], int aI[], int col=0, double elevent=0.0){
	/**
		\version	0.0.0.1
		\date		2014.02.26
		\author		Morochin <artamir> Artiom
		\details	ѕоиск самого первого совпадени€.
		\internal
			>Hist:	
